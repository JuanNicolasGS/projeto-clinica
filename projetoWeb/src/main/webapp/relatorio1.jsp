...
<head>
    <meta charset="UTF-8" />
    <title>Relatório 1 - Consultas por Clínica e Período</title>
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/index.css" />
</head>
<body>
<div class="form-container" style="max-width: 1100px;">
    <h1>Relatório 1</h1>
    <p>INNER JOIN (4 tabelas) + WHERE (clínica e período)</p>

    <c:if test="${not empty erro}">
        <div style="background:#ffebee;padding:10px;border-left:4px solid red;margin-bottom:15px;">
                ${erro}
        </div>
    </c:if>

    <form action="${pageContext.request.contextPath}/relatorio1" method="GET"
          style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:20px;">
        <input name="idClinica" placeholder="idClinica (ex: 1)" style="padding:12px;border:2px solid #ddd;border-radius:8px;">
        <input type="date" name="dataIni" style="padding:12px;border:2px solid #ddd;border-radius:8px;">
        <input type="date" name="dataFim" style="padding:12px;border:2px solid #ddd;border-radius:8px;">
